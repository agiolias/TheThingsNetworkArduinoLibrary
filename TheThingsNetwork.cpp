...
void TheThingsNetwork::deepSleep(int milliSeconds)
{
  String cmd = "";
  cmd.concat(F("sys sleep "));
  cmd.concat(milliSeconds);
  modemStream->println(cmd);
}

bool TheThingsNetwork::fromDeepSleep()
{
  modemStream->flush();
  return true;
}

bool TheThingsNetwork::wakeUp()
{
   // calling autoBaud will wake the device up.
   return setBaudRate(true) != "";
}

String TheThingsNetwork::setBaudRate(bool forWakeUp) {
  debugPrint(F("setBaudRate> "));
  ...
  {
    delay(1000);
    SoftwareSerial *s = (SoftwareSerial *)modemStream;
    ...
    if (forWakeUp)
    {
      response = readLine(1000);
      response = "OK"; // fake
    }
    else
    {
      modemStream->println("sys get ver");
      response = readLine(3000);
    }
  }
